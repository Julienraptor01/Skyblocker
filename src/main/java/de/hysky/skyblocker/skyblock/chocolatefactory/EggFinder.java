...
public class EggFinder {
	...
	private static final EnumSet<Location> possibleLocations = EnumSet.of(
	...
	public static void init() {
		...
		ClientCommandRegistrationCallback.EVENT.register((dispatcher, registryAccess) -> dispatcher.register(
				literal(SkyblockerMod.NAMESPACE).then(literal("eggFinder").then(literal("shareLocation")
						.then(argument("x", integer()).then(argument("y", integer()).then(argument("z", integer())
								.then(argument("eggType", word())
										.executes(context -> {
											MessageScheduler.INSTANCE.sendMessageAfterCooldown(
													String.format(
															"[Skyblocker] Chocolate %s Egg found at %d %d %d!",
															context.getArgument("eggType", String.class),
	...
	public static void checkIfEgg(Entity entity) {
		if (entity instanceof ArmorStandEntity armorStand)
			checkIfEgg(armorStand);
	}

	public static void checkIfEgg(ArmorStandEntity armorStand) {
		if (!SkyblockerConfigManager.get().helpers.chocolateFactory.enableEggFinder || SkyblockTime.skyblockSeason.get() != SkyblockTime.Season.SPRING || armorStand.hasCustomName() || !armorStand.isInvisible() || !armorStand.shouldHideBasePlate())
			return;
		if (Utils.getLocation() == Location.UNKNOWN) {
			//The location is unknown upon world change and will be changed via /locraw soon, so we can queue it for now
	...
	private static void renderWaypoints(WorldRenderContext context) {
		if (SkyblockerConfigManager.get().helpers.chocolateFactory.enableEggFinder)
			EggType.entries.parallelStream()
					.map(eggType -> eggType.egg.getValue())
					.filter(egg -> egg != null && egg.waypoint.shouldRender())
					.forEach(egg -> egg.waypoint.render(context));
	}
