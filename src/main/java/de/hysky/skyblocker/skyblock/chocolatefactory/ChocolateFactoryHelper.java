...
public class ChocolateFactoryHelper extends ContainerSolver {
	...
	private static final Pattern REQUIRED_PRESTIGE_CHOCOLATE = Pattern.compile("^Requires (" + NUMBER + "[a-zA-Z]?) Chocolate this Prestige!$");
		...
		if (!maxPrestige) {
			...
			for (Text line : ItemUtils.getLore(slots.get(prestigeSlot.intValue()))) {
				...
				try {
					...
					} else if (requiredMatcher.matches())
	...
	public static final class Tooltip extends TooltipAdder {
		...
		public void addToTooltip(Slot focusedSlot, ItemStack stack, List<Text> lines) {
			...
			if (prestigeSlot != null && focusedSlot.id == prestigeSlot && chocolateRequiredForPrestige != null) {
				NEW_LORE_LINES.add(Text.literal("Time until prestige: ").formatted(Formatting.GRAY).append(formatTime(chocolateRequiredForPrestige / (rawCps * correctedMultiplier))));
